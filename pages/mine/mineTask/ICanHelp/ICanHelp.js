...
Component({
  ...
  methods: {
    ...
    //执行者取消任务
    helperCancel(e) {
      cancelTaskInfo.taskId = e.currentTarget.id
      cancelTaskInfo.tid = e.currentTarget.dataset.tid
      cancelTaskInfo.index = e.currentTarget.dataset.index
      this.setData({isHiddenInput: false})
    },
    ...
    cancelTask(data) {
      ...
      console.log(cancelTaskInfo)
      wx.request({
        url: app.globalData.url + '/task/cancelTask/node',
        method: "POST",
        data: {
          taskId: cancelTaskInfo.taskId,
          tId: cancelTaskInfo.tid,
          openId: app.getOpenid(app.globalData.urlb),
          cancelReason: data.detail.text
        },
        success: res => {
          if (res.statusCode == 200) {
            console.log(res, "执行者取消任务成功返回的值-----------------------")
            wx.showToast({
              title: '取消任务成功',
              icon: 'SUCCESS'
            })
            //改变一个卡片的状态
            var taskList = that.data.taskList;
            taskList[cancelTaskInfo.index].status = 2;
            this.setData({
              taskList: taskList
            })
          } else {
